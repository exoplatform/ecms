...
//                        "      \"@microsoft.graph.conflictBehavior\" : \"rename\"\n" +
        ...
        if (status.equalsIgnoreCase("failed")) {
            Set<String> fileNames = getFiles(parentId, isFile ? fileName.substring(0, fileName.lastIndexOf('.')) : fileName).stream().filter((item) -> isFile == (item.file != null)).map((item) -> item.name).collect(Collectors.toSet());
            if (!fileNames.contains(fileName)) { // This means that the error is not related to the fact that a file with the same name exists.
...
//  }
    ...
    private List<DriveItem> getFiles(String folderId, String startsWith) {
        ...
        if (folderId == null) {
            iDriveItemCollectionPage = graphClient.me().drive().root().children().buildRequest(Collections.singletonList(deltaTokenQuery)).get();
        } else {
            iDriveItemCollectionPage = graphClient.me().drive().items(folderId).children().buildRequest(Collections.singletonList(deltaTokenQuery)).get();
        }
...
//                        .post().uploadUrl;
    ...
    public String insertUploadUrl(String parentId, String name) throws IOException, RefreshAccessException {
        ...
        String request = "{\n" +
                ...
                "  }\n" +
                "}";
        HttpPost httppost = new HttpPost("https://graph.microsoft.com/v1.0/me/drive/items/" + parentId + ":/" + name + ":/createUploadSession");
        StringEntity stringEntity = new StringEntity(request, "UTF-8");
        httppost.setEntity(stringEntity);
        httppost.addHeader("Authorization", "Bearer " + getAccessToken());
        httppost.addHeader("Content-type", "application/json");
...
//            FileSendResponse fileSendResponse = sendFile(uploadUrl, from, fileSlice.length, file.length, fileSlice);
//
//            DriveItem driveItem = getDriveItemIfCreated(fileSendResponse);
//            if (driveItem != null) {
//                return driveItem;
...
//  private DriveItem processEndOfFileUploadIfReached(FileSendResponse fileSendResponse) {
...
//    }
    ...
    public DriveItem insert(String parentId,
                            String fileName,
                            Calendar created,
                            Calendar modified,
    ...
    public DriveItem updateFileContent(String itemId,
                                       Calendar created,
                                       Calendar modified,
                                       InputStream inputStream) throws Exception {
