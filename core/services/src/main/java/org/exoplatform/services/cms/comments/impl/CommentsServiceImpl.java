...
public class CommentsServiceImpl implements CommentsService {
      ...
      if (listenerService_ != null) {
        try {
          if (activityService.isAcceptedNode(document)
            ...
            listenerService_.broadcast(ActivityCommonService.COMMENT_ADDED_ACTIVITY, document, newComment);
          }
        } catch (Exception e) {
  ...
  public void updateComment(Node commentNode, String newComment) throws Exception {
    ...
    if (listenerService_ != null && activityService != null) {
      try {
        if (activityService.isAcceptedNode(documentNode) ||
          ...
          listenerService_.broadcast(ActivityCommonService.COMMENT_UPDATED_ACTIVITY, documentNode, commentNode);
        }
      } catch (Exception e) {
        if (LOG.isErrorEnabled()) {
    ...
    if (listenerService_ != null && activityService != null) {
      ...
      try {
        if (activityService.isAcceptedNode(parentNode) ||
          ...
          listenerService_.broadcast(ActivityCommonService.COMMENT_REMOVED_ACTIVITY, commentNode, null);
        }
      } catch (Exception e) {
        if (LOG.isErrorEnabled()) {
  ...
  public List<Node> getComments(Node document, String language) throws Exception {
    ...
    if (!isSupportedLocalize(document, language)) {
      if (document.hasProperty("exo:language"))
    ...
    Session systemSession = WCMCoreUtils.getSystemSessionProvider()
                                        .getSession(session.getWorkspace().getName(),
  ...
  /**
   * This Class implements Comparator<Node> to compare the created date of
   * nodes.
   */
  ...
  private boolean isSupportedLocalize(Node document, String language) throws Exception {
    ...
    if (Collections.frequency(locales, language) > 0)
      return true;
    return false;
  }
