...
public class SiteSearchServiceImpl implements SiteSearchService {
  ...
  public SiteSearchServiceImpl(LivePortalManagerService portalManagerService,
    ...
    this.foundNodeCache = caService.getCacheInstance(SiteSearchService.class.getName()+"-founded");
    this.dropNodeCache = caService.getCacheInstance(SiteSearchService.class.getName()+"-dropped");
    if (initParams != null) {
...
//    System.out.println(queryStatement);
  ...
  private void mapQueryTypes(final QueryCriteria queryCriteria, final SQLQueryBuilder queryBuilder) throws Exception {
    ...
    queryBuilder.closeGroup();
    String[] excludedContentTypes = queryCriteria.getExcludedContentTypes();    
    if(excludedContentTypes != null && excludedContentTypes.length > 0) { 
      queryBuilder.openGroup(LOGICAL.AND_NOT); 
      queryBuilder.equal(NodetypeConstant.JCR_PRIMARY_TYPE,excludedContentTypes[0],LOGICAL.NULL);
      for(int i=1; i<excludedContentTypes.length; i++) {
        queryBuilder.equal(NodetypeConstant.JCR_PRIMARY_TYPE, excludedContentTypes[i], LOGICAL.OR);
      }
      queryBuilder.closeGroup();
    }
