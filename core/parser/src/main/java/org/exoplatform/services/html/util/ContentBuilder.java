...
public class ContentBuilder
{
   ...
   private void add(HTMLNode node, List<char[]> values, char[] chars)
   {
      ...
      if (values.size() > 0 && isEnd(values.get(values.size() - 1)) && length < 5)
         return;
      ...
      if (length < 11 && CharsUtil.indexOf(chars, month.toCharArray(), 0) > -1
         && CharsUtil.indexOf(chars, year.toCharArray(), 0) > -1)
         return;
      values.add(chars);
   }
