...
export default {
  props: {
    ...
    defaultDrive: { // all cloud drives that is in progress with their progress values recieved from parent
      type: Object,
      default: () => ({})
    }
  },
  ...
  methods: {
    initDestinationFolderPath: function () {
      ...
      this.$attachmentsService.getSpaceById(spaceId).then(space => {
        ...
        } else if (this.defaultDrive.name) {
          this.openDrive(this.defaultDrive).then(() => {
            const documentsFolder = this.folders.find(folder => folder.title === 'Documents');
            this.openFolder(documentsFolder).then(() => {
              this.$root.$emit('attachments-default-folder-path-initialized', this.selectedFolderPath, this.schemaFolder);
            });
          });
        } else {
      ...
      if (this.creatingNewFolder) {
        if (this.newFolderName) {
          ...
          if (folderNameExists) {
            ...
            const self = this;
            this.$attachmentsService.createFolder(this.currentDrive.name, this.workspace, this.currentAbsolutePath, this.newFolderName).then(xml => {
