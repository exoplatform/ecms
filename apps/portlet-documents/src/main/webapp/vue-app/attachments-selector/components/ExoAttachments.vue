<template>
  <exo-drawer
    id="exoAttachmentsApp"
    ref="attachmentsDrawer"
    class="attachments"
    body-classes="decrease-z-index"
    right>
      ...
      <v-flex class="mx-0 drawerHeader flex-grow-0">
        <v-list-item
          ...
          <v-list-item-action class="drawerIcons align-end d-flex flex-row">
            <v-btn icon @click="toggleServerFileSelector()">
          ...
          <v-list-item-action v-if="cloudDriveConnecting" class="drawerIcons align-end d-flex flex-row">
            <v-progress-linear
              absolute
              bottom
              indeterminate />
...
<script>
// TODO need to rework this to not use axios anymore 
...
export default {
        ...
        if (this.value.length > 0 && !this.pathDestinationFolder) {
          for (let i = 0; i < this.value.length; i++) {
            if (!this.value[i].pathDestinationFolder) {
              // TODO need to rework this to make this file smaller and more clean 
  ...
  methods: {
    ...
    removeAttachedFile: function(file) {
      if (!file.id) {
        this.$emit('input', this.value.filter(attachedFile => attachedFile.uploadId !== file.uploadId));
        ...
        this.$emit('input', this.value.filter(attachedFile => attachedFile.id !== file.id));
      }
    },
