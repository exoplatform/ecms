(function(gj, base, wcm_utils, ecm_utils) {
  var TimelineView = function() {

    // eXo.ecm.UITimelineView
    
    var Self = this;
    var BROW = eXo.core.Browser;
    
    TimelineView.prototype.temporaryItem = null;
    TimelineView.prototype.itemsSelected = [];
    TimelineView.prototype.allItems = [];
    TimelineView.prototype.contextMenuId = null;
    TimelineView.prototype.actionAreaId = null;
    TimelineView.prototype.enableDragDrop = null;

    TimelineView.prototype.colorSelected = "#e7f3ff";
    TimelineView.prototype.colorHover = "#f2f8ff";
    
    TimelineView.prototype.t1 = 0;
    TimelineView.prototype.t2 = 0;
    
    //init event
    TimelineView.prototype.initAllEvent = function(actionAreaId, enableDragAndDrop) {
  ...
  //    }
    ...
    TimelineView.prototype.initDragDropForTreeEvent = function(actionAreaId, enableDragAndDrop) {
      //registry action drag drop in tree list
      eXo.ecm.UITimelineView.enableDragAndDrop = enableDragAndDrop;
  ...
  //            }
    ...
    //event in tree list
    TimelineView.prototype.mouseOverTree = function(event) {
      ...
      scroller.onmousemove = eXo.ecm.UITimelineView.setScroll ;
    };
    
    TimelineView.prototype.setScroll = function(evt){
      if(Self.enableDragDrop) {
      eXo.ecm.UITimelineView.object = this;
        var element = eXo.ecm.UITimelineView.object ;
    ...
    TimelineView.prototype.mouseOutTree = function(event) {
      var element = this;
  ...
  //    Self.clickItem(event, element);   
    ...
    //event in item
    TimelineView.prototype.mouseOverItem = function(event) {
      var event = event || window.event;
      var element = this;
      if (!element.selected) {
    ...
    TimelineView.prototype.mouseOutItem = function(event) {
      var event = event || window.event;
      var element = this;
    ...
    TimelineView.prototype.dragItemsSelected = function(event) {
        var event = event || window.event;
        document.onselectstart = function(){return false;}
        if (eXo.ecm.UITimelineView.enableDragAndDrop != "true")
          return;
    ...
    TimelineView.prototype.dropOutActionArea = function(event) {
      var event = event || window.event;
      Self.enableDragDrop = null;
      revertResizableBlock();
    ...
    TimelineView.prototype.clickItem = function(event, element, callback) {
      var event = event || window.event;
      resetArrayItemsSelected();
    ...
    TimelineView.prototype.mutipleSelect = function(event) {
      var event = event || window.event;
      var element = this;
      var mask = gj(element).find("div.Mask:first")[0];
    ...
    TimelineView.prototype.showGroundContextMenu = function(event, element) {
      var event = event || window.event;
      resetArrayItemsSelected();
      if (document.getElementById(Self.contextMenuId)) {
        var contextMenu = document.getElementById(Self.contextMenuId);
    ...
    // hide context menu
    TimelineView.prototype.hideContextMenu = function() {
      var contextMenu = document.getElementById(Self.contextMenuId);
    ...
    TimelineView.prototype.postGroupAction = function(moveActionNode, ext) {
      var objectId = [];
      var workspaceName = [];
      var islink = "";
      var ext = ext? ext : "";
    ...
    TimelineView.prototype.setHeight = function() {   
      ...
      if (!container && uiResizableBlock) {
        ...
        if (container)
          eXo.ecm.UITimelineView.initialHeightOfContainer = container.offsetHeight;
      } 
    ...
    TimelineView.prototype.hideColumn = function(obj, event) {
      var event = event || window.event;
      event.cancelBubble = true;
    ...
    TimelineView.prototype.resizeColumn = function(obj, event) {
    ...
    TimelineView.prototype.resizeMouseMoveTimelineView = function(event) {
      var event = event || window.event;
      var objResize = eXo.ecm.UITimelineView.objResize;
      var objResizeClazz = eXo.ecm.UITimelineView.objRowClazz;
      var resizeValue = event.clientX - eXo.ecm.UITimelineView.currentMouseX;   
      var listGrid = gj(objResize).parents(".UIListGrid:first")[0];  
          
      // Case of resize width lower than allowable minimum.   
      if (eXo.ecm.UITimelineView.objResizeValue + resizeValue < 8 ) return;       
    ...
    TimelineView.prototype.resizeMouseUpTimelineView = function(event) {
      ...
      var objResizeClazz = eXo.ecm.UITimelineView.objRowClazz;    
      var resizeValue = event.clientX - eXo.ecm.UITimelineView.currentMouseX; 
      if (eXo.ecm.UITimelineView.objResize) {
        var objResize = eXo.ecm.UITimelineView.objResize;   
        objResize.style.width = eXo.ecm.UITimelineView.objResizeValue + resizeValue + "px";   
      
        if(eXo.ecm.UITimelineView.mapColumn.get(objResize.className)) {
          eXo.ecm.UITimelineView.mapColumn.remove(objResize.className);
          eXo.ecm.UITimelineView.mapColumn.put(objResize.className, eXo.ecm.UITimelineView.objResizeValue + resizeValue + "px");
        } else {
          eXo.ecm.UITimelineView.mapColumn.put(objResize.className, eXo.ecm.UITimelineView.objResizeValue + resizeValue + "px");
        }
      } 
      ...
      for (var i in objResizeClazz) {
        try {
          ...
          objColumn.style.width = eXo.ecm.UITimelineView.objResizeValue + resizeValue + "px";
        } catch(err) {
        }
      }
      ...
      document.onmousemove = null;
      delete eXo.ecm.UITimelineView.currentMouseX;
      delete eXo.ecm.UITimelineView.objResize;    
      delete eXo.ecm.UITimelineView.objClumnResize;
      delete eXo.ecm.UITimelineView.widthRightContainer;            
    } 
    
    TimelineView.prototype.loadEffectedWidthColumn = function() {
      var objResizeClazz = eXo.ecm.UITimelineView.objRowClazz;
  ...
  return {
    UITimelineView : eXo.ecm.UITimelineView
  };
})(gj, base, wcm_utils, ecm_utils);
